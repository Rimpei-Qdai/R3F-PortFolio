...
const Firework = ({ isMobile = false }) => {
  ...
  // 馬のシルエット形状を定義（正規化座標 -1 to 1）
  const getHorseShape = () => {
    const points = [];
    
    // 頭部
    for (let i = 0; i < 30; i++) {
      const angle = (i / 30) * Math.PI * 0.5 - Math.PI * 0.25;
      points.push({
        x: 0.5 + Math.cos(angle) * 0.3,
        y: 0.6 + Math.sin(angle) * 0.3,
        z: 0
      });
    }
    ...
    for (let i = 0; i < 8; i++) {
      ...
      points.push({
        x: 0.4 + t * 0.1,
        y: 0.85 + Math.sin(t * Math.PI) * 0.15,
        z: 0
      });
    }
  ...
  const createFirework = (position) => {
    const horsePoints = getHorseShape();
    const count = horsePoints.length * 2; // 各ポイントに複数のパーティクル
    const scale = 1.5 + Math.random() * 0.5; // スケール調整
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 馬の形状ポイントから取得（ランダムに選択）
      const shapePoint = horsePoints[Math.floor(Math.random() * horsePoints.length)];
      
      // 少しランダムなばらつきを追加
      const randomOffset = 0.05;
      const particlePosition = new THREE.Vector3(
        shapePoint.x * scale + (Math.random() - 0.5) * randomOffset,
        shapePoint.y * scale + (Math.random() - 0.5) * randomOffset,
        shapePoint.z * scale + (Math.random() - 0.5) * randomOffset
      );
